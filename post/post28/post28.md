...
To start, make sure you have nodejs installed, for demo purpose, I am using express to generate web scaffold, feel free to choose other types of frameworks as long as it is able to provide endpoints that is capable of returning JSON.
...
~~~
The initial data is populated by doing an ajax request to the backend. Then the _onchnage event is triggered so that the internal state of this component and that stored in its corresponding store are updated.

For the rest of the components, we just need to subclass this basecomponent class, and provide a customized event handler as shown below:
