...
def insert_into_db(data):
    ...
    if cursor.count() == 0:
        ...
        raise Exception("Got more than one existing entries, "
                        "did you submit two blogs with same content in the same day?")
    else:
        logger.warn("The given post was already in the db")
        result = cursor[0]
        confirm = input("Are you sure you want to override the current content? id: "
                        + str(result["id"]) + ": y/N")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process blog entry, you need to have all of "
                                     "the following files: post.json, post.md in the given folder")
    ...
    parser.add_argument('--update','-u', action='store_true',
                        help="Flag to indicate that this will update the correspoing entry, need to enter the id as argument. You must specify post to use this option")
    parser.add_argument('--show', '-s', action='store_true',
                        help="Show all the entries currently in the db")
    parser.add_argument('--preview', '-v', action='store_true',
                        help="Flag to indicate that we want to see the parsed json")
    parser.add_argument('--delete', '-d', type=int, help="Delete one entry")
    parser.add_argument('--deleteALL', action='store_true',
                        help="Delete ALL entries(USE CAUTION!)")
